...
- #3/#10/#11/#13 now have Claude final-review acceptance in `.agent-bridge/TO_CODEX.md` on 2026-07-05. #3/#11/#13 close as `V1 Done with known limitation`; #10 closes as `Done` for V1 academic source breadth. The final sprint gate still must be rerun before any completion claim.
...
| 3 | Need broader, higher-quality news/source coverage and local pre-analysis. | `V1 Done with known limitation`. Claude final review accepted the first classified fresh-source batch as the repair-sprint V1. | `backend/config/feeds.json` now has 25 curated feeds, including 8 `fresh_rss` + `public` sources: UN News, NPR World, The Conversation, CNBC World, The White House, Federal Reserve, European Central Bank, and WTO News; source registry persists and returns `coverage/access/last_tested/coverage_reason/state_media`; source manager summarizes total/enabled/failed/limited sources, latest successful fetch, failed-source reasons, source type mix, quality-tier mix, and classified coverage fields; collection no longer falls back to disabled/limited registry rows; `backend/tests/test_source_registry.py` -> `11 passed`; `backend/tests/test_topic_ops.py tests/test_api_helpers.py` -> `13 passed`; backend full pytest -> `218 passed, 5 warnings in 36.44s`; frontend source-manager e2e evidence is recorded above; Claude accepted this scope in `.agent-bridge/TO_CODEX.md` on 2026-07-05. | Known limitations: not a full crawler; same-event G20 coverage is not guaranteed; WSJ/AFP/Xinhua public feeds remain stale or license/API-bound; IMF/OECD/World Bank/State Dept/Reuters replacement URLs, multilingual sources, and paywalled/API integrations are future batches. |
...
| #3 news/source quality | Source registry, source manager status, failure reasons, evidence package tests, disabled/limited collection boundary, and the first classified fresh-source batch exist. Current tree evidence: 25 curated feeds; 8 fresh/public classified feeds; backend source-registry tests `11 passed`; backend full pytest `218 passed, 5 warnings in 36.44s`; Claude accepted the V1 limitation boundary on 2026-07-05. | Fresh final gate after status/doc updates. | Final gate. |
| #6 media/community trend semantics | Frontend trend and sentiment timeline e2e pass, with share changes, small-sample downgrade, and tiny-bucket markers. Codex semantic scan found the UI explicitly says `报道样本`, `不代表民间舆论`, `当前样本只能显示立场分布`, `样本趋势，非事实时间线`, and `小样本线索`; tests assert the key boundaries; Claude semantic review PASS found no pseudo-trend red-line issue. | Fresh final gate after status/doc updates. | Final gate. |
